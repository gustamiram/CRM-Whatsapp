...
export interface Contact {
  ...
  /** WhatsApp LID (migration 038), set only when UAZAPI never resolved
   *  a real phone number for this contact — `phone` then holds the
   *  LID's digits (not dialable) and sends route via `{wa_lid}@lid`
   *  instead. Always null for Meta contacts. */
  wa_lid?: string | null;
  name?: string;
  email?: string;
  company?: string;
