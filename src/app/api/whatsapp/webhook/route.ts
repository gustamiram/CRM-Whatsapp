...
export async function POST(request: Request) {
  ...
  // guaranteeing the work runs to completion. `after()` keeps the
  // function alive until the callback resolves (see issue #301).
  after(async () => {
    try {
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      // Template-lifecycle events come in on a different change.field and
      // have a different value shape — route them to the dedicated handler.
      ...
      // PGRST116 for both 0 rows AND ≥2 rows — fetch the list so we can
      // distinguish and log the real cause.
      ...
      for (let i = 0; i < value.messages.length; i++) {
        ...
        await processMetaMessage(
          ...
          config.account_id,
          // Audit / sender-of-record for NOT NULL user_id FK inserts.
          config.user_id,
...
/**
 * Translate one Meta inbound message into the provider-neutral inbound
 * pipeline. Meta-specific work (media-id → proxy URL, interactive-reply
 * shape) happens here in `parseMessageContent`; everything downstream is
 * shared with the UAZAPI provider via `ingestInboundMessage`.
 */
async function processMetaMessage(
  ...
  // Reactions aren't messages — hand them to the core's reaction path.
  ...
  // mediaType is only used to construct the proxy URL above; the schema
  // has no media_type column. Silence the unused-var warning.
  void mediaType
...
async function parseMessageContent(
   ...
   * `messages.interactive_reply_id`. Null for everything else.
   */
  ...
  switch (message.type) {
    ...
    case 'sticker':
      // Stickers are images under the hood. The core maps the DB
    ...
    case 'interactive': {
      // The customer tapped a reply button or a list row. Use the
      // human-readable title as contentText and stash the stable id.
      const reply =
